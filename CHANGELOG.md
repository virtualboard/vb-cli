...
- Enhanced `vb validate --fix` to automatically sync filenames with feature titles
- Added `RenameToMatchTitle` method to feature manager for safe atomic file renaming

### Changed

- Validator now enforces that filenames match the pattern `{ID}-{slugified-title}.md`
- `vb validate --fix` now performs both template fixes and filename syncing
